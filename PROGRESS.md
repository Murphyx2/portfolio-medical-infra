...
Verified against the live stack on 2026-08-10 (perf pass + UX/a11y pass); repo commit hashes re-checked 2026-08-11. All three repos clean (`git status` empty on backend/frontend; infra has one harmless untracked `.claude/` dir from local agent tooling), all on `main`.

### Repos & latest commits
| Repo       | Path       | Latest commit |
|------------|------------|----------------|
| infra      | `infra/`   | `0394ed9` |
| backend    | `backend/` | `30a8c50` (merge of `perf/caching-and-query-opts`, tip `824410f`) |
| frontend   | `frontend/`| `c8b64e8` (merge of `ux/impeccable-critique-fixes`, tip `bc9ba02`) |
...
- Backend tests: `pytest` (from `backend/`; last verified **210 passed**, 2026-08-10).
- Frontend build: `npm run build` (from `frontend/`). Frontend tests: `npm test` (last verified **44 passed**, 2026-08-10).
