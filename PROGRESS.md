...
Everything below was verified against the live stack. All three repos are clean (`git status` empty), all on `main`; the frontend UX/a11y fix branch was merged `--no-ff` and deleted.
...
- Frontend tests: `npm test` (vitest + RTL; expect **44 passed**).
...
- **Vite stale-cache on Docker bind-mount:** after changing frontend code or env, the container may keep serving old transformed modules. Root cause of the AuthProvider crash and the `ERR_NAME_NOT_RESOLVED` login bug — **recurred again on 2026-08-10** during the UX/a11y fix pass, where the container silently served pre-session code through ~10 commits' worth of changes while `curl` kept returning `200`. **Fix: recreate the container** — `docker compose up -d --force-recreate frontend` (or at least `up -d`), NOT plain `docker compose restart`. **Don't trust HTTP status codes as proof of freshness** — verify by diffing the actual served file content (or grepping for a source-only identifier) against current source.
...
7. **Sticky table headers (deferred, 2026-08-10 UX pass):** reverted after discovering `.data-table`'s `overflow: hidden` (rounded corners) makes it a scroll-container ancestor that breaks page-relative `position: sticky` — see the Progress Log entry above. Doing this properly means giving `.data-table` its own bounded `max-height` + `overflow-y: auto` (an independently-scrolling table box instead of whole-page scroll), which is a real UX decision, not a CSS tweak — worth a deliberate call before attempting again.
8. **`theme/palette.ts` is dead code with stale colors:** unused anywhere in `frontend/src` (verified during the 2026-08-10 critique) but still hardcodes the pre-contrast-fix `#66BB6A` green. Delete it, or sync it to `index.css`'s current tokens if something ends up needing a JS-side color source (e.g. a future chart).
