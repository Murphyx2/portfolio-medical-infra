...
| frontend   | `frontend/` (React SPA)                                                                         | `f997165`     |

### Runbook
- Start stack: `docker compose up -d` (run from `infra/`). Services: `mc_db`, `mc_cache`, `mc_backend`, `mc_frontend`.
- Backend tests: `.\backend\.venv\Scripts\python.exe -m pytest` (run from `backend/`; expect **150 passed**).
- Frontend build: `npm run build` (run from `frontend/`).
- Frontend tests: `npm test` (vitest + RTL; expect **23 passed**).
