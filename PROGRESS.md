...
**2026-09-04 — dev compose: backend bound to loopback, frontend confirmed LAN-open by design** — infra `3c3b9d2` (backend fix) + same-day follow-up (frontend revert). A 3-agent architecture/software audit flagged both `backend`/`frontend` publishing on `0.0.0.0`; `backend` is now `127.0.0.1:8000:8000`, matching `db`/`cache` (API isn't meant to be hit directly — the frontend's same-origin proxy is the intended path). `frontend` was reverted back to `5173:5173` (all interfaces) after explicit confirmation: being reachable from any device on the LAN/Wi-Fi is the *intended* behavior for the dev frontend, not a gap — see `docs/docker-management.md` §4 for the still-required host firewall step and its security notes (Private-profile-only, never port-forwarded, HTTP-only so don't leave it running longer than needed).
